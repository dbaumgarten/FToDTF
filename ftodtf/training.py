...
def train(settings):
    ...
    server, _ = _tfserver_from_settings(settings)
    ...
    hooks = [tf.train.StopAtStepHook(last_step=settings.steps)]
    ...
    with m.graph.as_default():
        with tf.train.MonitoredTrainingSession(
                ...
                hooks=hooks,
                save_checkpoint_steps=10000) as session:
            ...
            try:
                ...
                while not session.should_stop():
                    ...
                    _, summary, loss_val, step_nr = session.run(
                        [m.optimizer, m.merged, m.loss, m.step_nr],
                    ...
                    if step % 10000 == 0 and settings.validation_words:
                        ftodtf.validation.print_similarity_check(
                            m.validation, settings.validation_words_list, session)
